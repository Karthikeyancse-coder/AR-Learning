...
const colors = {
  cyan: {
    stripe:    "#0da6f2",
    cardBg:    "linear-gradient(135deg,#e8f7ff 0%,#f0faff 100%)",
    trackBg:   "#bee8fb",
    bar:       "#0da6f2",
    barGlow:   "0 0 8px rgba(13,166,242,0.45)",
    badgeBg:   "rgba(13,166,242,0.12)",
    badgeText: "#0284c7",
    text:      "#0da6f2",
    btnBg:     "rgba(13,166,242,0.12)",
    btnText:   "#0284c7",
    btnHover:  "#0da6f2",
    border:    "#bae6fd",
    hover:     "#7dd3fc",
  ...
  purple: {
    stripe:    "#7B61FF",
    cardBg:    "linear-gradient(135deg,#f0eeff 0%,#f5f3ff 100%)",
    trackBg:   "#ddd6fe",
    bar:       "#7B61FF",
    barGlow:   "0 0 8px rgba(123,97,255,0.45)",
    badgeBg:   "rgba(123,97,255,0.12)",
    badgeText: "#6d28d9",
    text:      "#7B61FF",
    btnBg:     "rgba(123,97,255,0.12)",
    btnText:   "#6d28d9",
    btnHover:  "#7B61FF",
    border:    "#c4b5fd",
    hover:     "#a78bfa",
  ...
  green: {
    stripe:    "#00b98a",
    cardBg:    "linear-gradient(135deg,#e6faf4 0%,#f0fdf9 100%)",
    trackBg:   "#a7f3d0",
    bar:       "#00b98a",
    barGlow:   "0 0 8px rgba(0,185,138,0.45)",
    badgeBg:   "rgba(0,185,138,0.12)",
    badgeText: "#059669",
    text:      "#00b98a",
    btnBg:     "rgba(0,185,138,0.12)",
    btnText:   "#059669",
    btnHover:  "#00b98a",
    border:    "#6ee7b7",
    hover:     "#34d399",
...
const Chemistry = () => {
  ...
  const { chapters, loading } = useAppSelector((state: RootState) => state.chapter);
          ...
          {tabs.map((tab, i) => (
            <button
              ...
              className={`px-6 py-2.5 text-xs font-bold uppercase tracking-widest transition-colors border-b-2 bg-transparent ${
                activeTab === i 
                  ? "text-[#0da6f2] border-[#0da6f2]" 
                  : "text-[#94a3b8] dark:text-slate-500 border-transparent hover:text-slate-500 dark:hover:text-slate-400"
              }`}
            >
              {tab}
            </button>
        ...
        <div className="mt-12">
          ...
          <div className="space-y-3">
            {chapters.map((item, i) => (
              <button
                key={item.id}
                onClick={() => navigate(`/dashboard/chemistry/${item.id}`)}
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-9 h-9 bg-slate-900 rounded-xl flex items-center justify-center text-white font-bold text-sm">
                    {i + 1}
                  </div>
                  <h4 className="font-medium text-slate-700 dark:text-white text-sm">{item.name}</h4>
                </div>
                <span className="material-symbols-outlined text-slate-400 hover:text-sky-500 transition-colors">
                  chevron_right
                </span>
              </button>
            ))}
  ...
  return (
    ...
    >
      ...
      <div className="relative h-48 w-full overflow-hidden">
        ...
        {/* Badge */}
        <div
          className="absolute top-3 right-3 px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wider dark:!bg-slate-800 dark:!text-white/90 dark:!border-slate-600"
          style={{
      ...
      <div className="p-5 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-1.5">
          <h3
            ...
            style={{ color: hovered ? c.stripe : undefined }}
          >
            {course.title}
          </h3>
        ...
        <p className="text-xs text-slate-500 dark:text-[#B0B3C0] mb-6 line-clamp-2 leading-relaxed">{course.desc}</p>
        ...
        <div className="mt-auto space-y-4">
          ...
          <div>
            <div className="flex justify-between text-[10px] font-bold uppercase mb-1.5 tracking-widest">
              ...
              <span style={{ color: c.stripe }}>{course.progress}%</span>
            </div>
            <div className="w-full h-2 rounded-full overflow-hidden dark:!bg-[#2a2a3a]" style={{ background: c.trackBg }}>
              <div
                className="h-full rounded-full transition-all duration-500"
                style={{ width: `${course.progress}%`, background: c.bar, boxShadow: c.barGlow }}
              />
            </div>
          </div>

          {/* Footer */}
          <div className="flex items-center justify-between text-xs text-slate-400 border-t pt-4" style={{ borderColor: c.border }}>
            ...
            <button
              ...
              style={{
                background: btnHovered ? c.btnHover   : c.btnBg,
                color:      btnHovered ? "#ffffff"    : c.btnText,
                boxShadow:  btnHovered ? c.barGlow    : "none",
              }}
            >
              Resume
...
const EnrollCard = () => {
  ...
  return (
    ...
    >
      <div
        className="size-20 rounded-full flex items-center justify-center mb-5 transition-all duration-300 dark:!bg-slate-800 dark:!border-slate-600"
        style={{
      ...
      >
        <span
          className={`material-symbols-outlined text-4xl transition-colors ${hovered ? "" : "dark:!text-slate-300"}`}
          style={{ color: hovered ? "#0da6f2" : "#94a3b8" }}
        >
          add
        </span>
      </div>
      <h3
        className={`text-sm font-bold uppercase tracking-wide transition-colors ${hovered ? "" : "dark:!text-white"}`}
        style={{ color: hovered ? "#0da6f2" : "#64748b" }}
      >
        Enroll New Module
      </h3>
